...
public class CustomAuth extends HttpServlet {
  ...
  protected boolean allowUser(String auth) throws IOException {
    ...
    System.out.println("加密: " + userpassEncoded);
    ...
    System.out.println("解密: " + userpassDecoded);

    // Check our user list to see if that user and password are "allowed"
    System.out.println(users.get(userpassDecoded));
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse res)
                               throws ServletException, IOException { //第1先跑doGet
    res.setContentType("text/plain");
    PrintWriter out = res.getWriter();

    // Get Authorization header
    // 取得client端請求標頭
    String auth = req.getHeader("Authorization"); //page72 + page79 

    // Do we allow that user?
    // 進入allowUser方法  
    if (!allowUser(auth)) {
      // Not allowed, so report he's unauthorized
      res.setHeader("WWW-Authenticate", "BASIC realm=\"users\""); //page72 + page95
      res.sendError(HttpServletResponse.SC_UNAUTHORIZED);         //page72 + page94
